...
public class ExtractDecodeEditEncodeMuxTest extends AndroidTestCase {
    ...
    private boolean mCopyEncodeBuffer; /*保存视频264或者265压缩流*/
    ...
    /** h264和h265编码的文件。有一些系统好像合成mp4会有问题，合成不了。退而求其次保存编码文件*/
    private String mOutputEncodeFile;
    private FileOutputStream mOutputEncodeFileStream;
    ...
    public void testExtractDecodeEditEncodeMuxVideo(String file, int width, int height, String outType, int fps, int bitRate, boolean saveEncode) throws Throwable {

        mOutputEncodeFile = null;
        mOutputEncodeFileStream = null;

        setSize(width,height);  //by ppym
        if(file != null)
            setSource(file);
        else
            setSource(R.raw.video_480x360_mp4_h264_500kbps_30fps_aac_stereo_128kbps_44100hz);
    ...
    private void setmCopyEncodeBuffer(){
        mCopyEncodeBuffer = true;
    }
    ...
    private void setOutputFile() {
        ...
        if(mCopyEncodeBuffer) {
            StringBuilder sb_temp = new StringBuilder(sb.toString());
            if(mOutputVideoMimeType.equals(OUTPUT_VIDEO_MIME_H264_TYPE)){
                sb_temp.append(".h264");
            }
            else{
            ...
            mOutputEncodeFile = sb_temp.toString();
        }
        sb.append(".mp4");
    ...
    private void extractDecodeEditEncodeMux() throws Exception {
        ...
        try {
            ...
            if (mCopyVideo) {
                if(mInputFile != null) {
                    mVideoExtractor = createExtractor(mInputFile);
                }else {
               ...
               // mOutputSurface.changeFragmentShader(FRAGMENT_SHADER); //by ppym
            ...
            try {
                if (null != mOutputEncodeFileStream) {
                    mOutputEncodeFileStream.close();
                }
            }catch (Exception e){
                Log.e(TAG, "error while close mOutputEncodeFileStream", e);
                if(exception == null) {
                    exception = e;
                }
            }
    ...
    /**
     * Creates an extractor that reads its frames from fileString
     */
    private MediaExtractor createExtractor(String fileString) throws IOException {
        MediaExtractor extractor;
        extractor = new MediaExtractor();
        try {
            extractor.setDataSource(fileString);
    ...
    private MediaCodec createVideoDecoder(MediaFormat inputFormat, Surface surface) throws IOException {
        ...
        MediaCodec.Callback callback = new MediaCodec.Callback() {
            public void onError(MediaCodec codec, MediaCodec.CodecException e) {
                Log.d("MediaCodecCallback","decode Errror mess: " + e);
                Log.d("MediaCodecCallback","decode Errror diag: " + e.getDiagnosticInfo());
                Log.d("MediaCodecCallback","decode Errror rec: " + e.isRecoverable());
                Log.d("MediaCodecCallback","decode Errror tra: " + e.isTransient());
            }
    ...
    private void setupMuxer() {
        if (!mMuxing
            ...
            if (mCopyVideo) {
                ...
                Log.d(TAG, "muxer: adding video track. mOutputVideoTrack " + mOutputVideoTrack + " " + mEncoderOutputVideoFormat);
            }
            if (mCopyAudio) {
    ...
    private void muxVideo(int index, MediaCodec.BufferInfo info) {
        ...
        if ((info.flags & MediaCodec.BUFFER_FLAG_CODEC_CONFIG) != 0) {
            ...
            if(mOutputEncodeFileStream != null){
                byte[] buf = new byte[info.size];
                encoderOutputBuffer.get(buf);
                try {
                    mOutputEncodeFileStream.write(buf);
                    mOutputEncodeFileStream.flush();
                }
                catch (IOException e){
                    Log.e(TAG,"write origin encode buffer error");
                }
            }
    ...
    private MediaMuxer createMuxer() throws IOException {
        if(mOutputEncodeFile != null){
            try {
                mOutputEncodeFileStream = new FileOutputStream(mOutputEncodeFile);
            }
            catch (Exception e){
                Log.e(TAG,"open " + mOutputEncodeFile + " error ");
                e.printStackTrace();
            }
        }
